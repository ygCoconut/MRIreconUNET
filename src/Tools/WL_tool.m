...
elseif strcmp(cmap2,'Less...')
	...
	% 		(['set(', num2str(userdata{1}) , ', ''colormap'' , colormap(' ,cmap2,'))' ]);
	% 		set(userdata{1},'colormap', colormap(  ));
	% 	else
	% 		eval(['set(', num2str(userdata{1}, '%1.15g') , ', ''colormap'' , colormap(' ,cmap2, '(' num2str(size(userdata{2},1)), ')))' ]);
	% 	end;
	% With:
	switch cmap2
		case 'Jet', cmap3 = jet(size(userdata{2},1));
		case 'Gray', cmap3 = gray(size(userdata{2},1));
		case 'Hsv', cmap3 = hsv(size(userdata{2},1));
		case 'Hot', cmap3 = hot(size(userdata{2},1));
		case 'Bone', cmap3 = bone(size(userdata{2},1));
		case 'Copper', cmap3 = copper(size(userdata{2},1));
		case 'Pink', cmap3 = pink(size(userdata{2},1));
		case 'White', cmap3 = white(size(userdata{2},1));
		case 'Flag', cmap3 = flag(size(userdata{2},1));
		case 'Lines', cmap3 = lines(size(userdata{2},1));
		case 'Colorcube', cmap3 = colorcube(size(userdata{2},1));
		case 'Prism', cmap3 = prism(size(userdata{2},1));
		case 'Cool', cmap3 = cool(size(userdata{2},1));
		case 'Autumn', cmap3 = autumn(size(userdata{2},1));
		case 'Spring', cmap3 = spring(size(userdata{2},1));
		case 'Winter', cmap3 = winter(size(userdata{2},1));
		case 'Summer', cmap3 = summer(size(userdata{2},1));
		case 'Vga', cmap3 = vga;
	end;
	set(userdata{1}, 'colormap', cmap3);
...
% Make Code "compilable"
% Above code replaces the following (eval based)
% for i = 1:length(s)
