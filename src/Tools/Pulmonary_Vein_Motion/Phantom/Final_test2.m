...
[V aR aT spacing info AngM OrM] = get_image(693,17,'C:\ThirdParty\RawData\Phantom_mehdi\',0);
Ra = [aR(1:3),aR(4:6)];
tempM = cross(aR(1:3),aR(4:6));
Ra = [Ra,tempM];
...
return
[Vi bR bT spacing1 info AngM OrM] = get_image(675,17,'C:\ThirdParty\RawData\Phantom_mehdi\',0);
...
OrgAli = aT(:,1);
%OrgAli(2) = aT(2,size(aT,2));  
OrgAli2 = bT(:,1);
%OrgAli2(2) = bT(2,size(bT,2));
for iz = 1: size(V,3)
     for iy = 1 : (sqrt(1)*size(V,2))
         for ix = 1: (sqrt(1)*size(V,1))
             
             temp(1,1) =  (ix-1)*spacing1(1,1);
             temp(2,1) =  (iy-1)*spacing1(2,1);
             temp(3,1) =  (iz-1)*spacing1(3,1);
            % [ix;iy;iz]
             tempOne = Rb*temp + OrgAli2;
%             [ix;iy;iz]
%             temp
...
%              temp1mm(3,1) = ((tempTwo(3,1)-CCz1)/spacing1(3,1))+1;
               temp1 = round(Ra'*(tempTwo-OrgAli)./spacing+1);
