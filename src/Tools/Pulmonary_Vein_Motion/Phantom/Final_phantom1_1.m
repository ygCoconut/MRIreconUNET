...
[V aR aT spacing info] = get_image(1579,33,'C:\ThirdParty\RawData\Mehdi_Phantom1\',0);%639
...
%              temp(3,1) = (iz-1)*spacing(3,1)/2 + aT(3,iz);
             ...
             temp1(1,1) = CCx + (ix-1)*spacing(1,1);
             temp1(3,1) = CCy + (iz-1)*spacing(2,1);
             temp1(2,1) = CCz + (iy-1)*info.SpacingBetweenSlices;
         
             %temp1 = Rb'*(temp-[meanx1;meany1;meanz1])+[meanx1;meany1;meanz1];
             temp = Rb'*temp1-Rb'*C;
             
             temp(1,1) = ceil((temp(1,1)-CCx)/spacing(1,1))+1;
             temp(2,1) = ceil(-(temp(2,1)+CCy)/spacing(2,1))+1;
             temp(3,1) = ceil((temp(3,1)-CCz)/(info.SpacingBetweenSlices))+1;
...
%           end
         ...
         %bias = min(min(temp);
          if ((temp(1,1)>0 && temp(1,1)<=size(V,1)) && (temp(2,1)>0&& temp(2,1)<=size(V,2)) && (temp(3,1)>0&& temp(3,1)<=size(V,3)))
             VV(ix,iy,iz) = V(temp(1,1),temp(2,1),temp(3,1));
