function out = myzoom(varargin)
%disp('called myzoom, good!!');
...
%   Double clicking zooms out to the point at which zoom was first 
%   turned on for this figure.  Note that turning zoom on, then off 
%   does not reset the zoom point.  This may be done explicitly with 
%   ZOOM RESET.
%   
...
%   $Revision: 5.53 $  $Date: 2000/08/04 22:29:33 $
...
%
%   Revised Jan 2001 for use with PZ_tool;
%   
%   By: Daniel Herzka  herzkad@nih.gov
...
if strcmp(zoomCommand,'off'),
   ...
   if ~isempty(state),
       % since we don't set the pointer, make sure it does not get reset
       ptr = get(fig,'pointer');
...
case 'on',
   ...
   if isempty(state),
       ...
       % restore button down functions for any other children of the figure
       uirestore(state,'children');
       setappdata(fig,'ZOOMFigureState',state);
   end

   set(findall(fig,'Tag','figToolZoomIn'),'State','on');
   set(fig,'windowbuttondownfcn','myzoom(gcbf,''down'')', ...
...
case 'xon',
   myzoom(fig,'on') % Set up userprop
   set(fig,'windowbuttondownfcn','myzoom(gcbf,''xdown'')', ...
      'windowbuttonupfcn','ones;', ...
...
case 'yon',
   myzoom(fig,'on') % Set up userprop
   set(fig,'windowbuttondownfcn','myzoom(gcbf,''ydown'')', ...
      'windowbuttonupfcn','ones;', ...
      'windowbuttonmotionfcn','','buttondownfcn','',...
      'interruptible','on');
   set(ax,'interruptible','on')
...
case 'getlimits', % Get axis limits
   ...
   %DAH - do not change axes limits if PZ tool is open... guarantees 
   % that if we start with a square axis, the zoom ends up square
   %if ~isempty(findobj('Name', 'PZ Tool'))
   %    xlim = get_xlim(ax); xmin = xlim(1); xmax = xlim(2); 
   %    ylim = get_ylim(ax); ymin = ylim(1); ymax = ylim(2); 
   %end;
...
list = myzoom(fig,'getconnect'); % Circular list of connected axes.
if zoomx,
