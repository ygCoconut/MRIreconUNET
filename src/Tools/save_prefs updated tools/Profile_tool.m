...
if (~isempty(hT.Current_Profile)) | (isempty(hT.Current_Profile) & (scope==ALL_PROFILES | scope == ALL_IMAGE_PROFILES) )
    ...
    case ALL_IMAGE_PROFILES 
        ...
        if ~isempty(hT.Current_Profile) & (Delete_Axes ~= hT.Current_Profile(2))
            disp(['Current Profile Still Exist']);
    ...
    for i = 1:size(hT.Profile_info_table,1)
        ...
        [hT.Profile_info_table(i,:).Profile_Exists]
...
if nargin==0
	...
	if strcmp(SelectionType,'alt')
		...
		Stretch_Current_Profile(hT.Profile_info_table(hT.Current_Profile(1),hT.Current_Profile(2)));	
		Update_Profile_Info([hT.Current_Profile(1),hT.Current_Profile(2)]);
		Update_Profile_Info_String([hT.Current_Profile(1),hT.Current_Profile(2)]);
		Resort_Profile_Info_Listbox;
		Update_Profile_Plot([hT.Current_Profile(1),hT.Current_Profile(2)]);
		figure(hT.Parent_figure);
		figure(hT.Tool_figure);
		figure(hT.Info_figure);
		figure(hT.Plot_figure);	
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function Update_Profile_Info(update_list)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:size(update_list,1)
    Profile_Statistics = [update_list(i,[2,1]), handlesTool.Profile_info_table(update_list(i,1), update_list(i,2)).Profile_Info];
...
for i = 1:size(handlesTool.Profile_info_table,1)
    for j = 1:size(handlesTool.Profile_info_table,2)
        if handlesTool.Profile_info_table(i,j).Profile_Exists
            Profile_Statistics = handlesTool.Profile_info_table(i,j).Profile_Info;
...
% don't shrink a dimension if there is only one axes
if grid_size(1)==1, shrinky = 1; end;
if grid_size(2)==1, shrinkx = 1; end;
...
for j = 1:grid_size(1)
    for i = 1:grid_size(2)
        if hT.h_all_axes(j,i)
            ...
            set(h_plot_axes(j,i), ...
                'Position', [pos(1)+(i-1)*w, pos(2)+pos(4)-j*h , w*shrinkx, h*shrinky]);
        end;                
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function Refresh_Profiles(Profile_table);
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h_end2 = plot(pos_x2 , pos_y2, [profile_color,'o'],...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function Stretch_Current_Profile(table_entry);
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Function to stretch or compress profiles upon right (alternate) clicking
%global DB; if DB disp(['Profile_tool: ', Get_Current_Function]); end;
